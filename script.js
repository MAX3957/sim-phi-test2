...
function loop() {
	...
	//Copyright
	ctx.font = `${lineScale * 0.4}px Saria,Noto Sans SC`;
	ctx.fillStyle = "#ccc";
	ctx.globalAlpha = 0.8;
	ctx.textAlign = "right";
...
function qwqdraw1(now) {
	...
	if (document.getElementById("showCE2").checked)
		for (const i of clickEvents2) { //绘制打击特效2
			...
			ctxos.font = `bold ${noteScale*(256+128* (((0.2078 * tick - 1.6524) * tick + 1.6399) * tick + 0.4988))}px Saria,Noto Sans SC`;
	...
	if (qwqIn.second >= 3 && qwqOut.second == 0) {
		if (showPoint.checked) { //绘制定位点
			ctxos.font = `${lineScale}px Saria,Noto Sans SC`;
			ctxos.textAlign = "center";
	...
	if (qwqIn.second < 3) {
		...
		ctxos.font = `${lineScale * 1.1}px Saria,Noto Sans SC`;
		const dxsnm = ctxos.measureText(inputName.value || inputName.placeholder).width;
		if (dxsnm > canvasos.width - lineScale * 1.5) ctxos.font = `${(lineScale) * 1.1/dxsnm*(canvasos.width-lineScale*1.5)}px Saria,Noto Sans SC`;
		ctxos.fillText(inputName.value || inputName.placeholder, wlen, hlen * 0.75);
		//曲绘和谱师
		ctxos.textBaseline = "top";
		ctxos.font = `${lineScale * 0.55}px Saria,Noto Sans SC`;
		const dxi = ctxos.measureText(`Illustration designed by ${inputIllustrator.value || inputIllustrator.placeholder}`).width;
		if (dxi > canvasos.width - lineScale * 1.5) ctxos.font = `${(lineScale) * 0.55/dxi*(canvasos.width-lineScale*1.5)}px Saria,Noto Sans SC`;
		...
		if (dxc > canvasos.width - lineScale * 1.5) ctxos.font = `${(lineScale) * 0.55/dxc*(canvasos.width-lineScale*1.5)}px Saria,Noto Sans SC`;
	...
	ctxos.textBaseline = "alphabetic";
	ctxos.font = `${lineScale * 0.95}px Saria,Noto Sans SC`;
	...
	if (stat.combo > 2) {
		ctxos.textAlign = "center";
		ctxos.font = `${lineScale * 1.32}px Saria,Noto Sans SC`;
		...
		ctxos.font = `${lineScale * 0.66}px Saria,Noto Sans SC`;
	...
	ctxos.textBaseline = "alphabetic";
	ctxos.textAlign = "right";
	ctxos.font = `${lineScale * 0.63}px Saria,Noto Sans SC`;
	const dxlvl = ctxos.measureText(inputLevel.value || inputLevel.placeholder).width;
	if (dxlvl > wlen - lineScale) ctxos.font = `${(lineScale) * 0.63/dxlvl*(wlen - lineScale )}px Saria,Noto Sans SC`;
	...
	ctxos.font = `${lineScale * 0.63}px Saria,Noto Sans SC`;
	const dxsnm = ctxos.measureText(inputName.value || inputName.placeholder).width;
	if (dxsnm > wlen - lineScale) ctxos.font = `${(lineScale) * 0.63/dxsnm*(wlen - lineScale )}px Saria,Noto Sans SC`;
	...
	if (qwq[0]) {
		...
		ctxos.textBaseline = "middle";
		ctxos.font = `${lineScale * 0.4}px Saria,Noto Sans SC`;
		ctxos.textAlign = "left";
...
function qwqdraw3(statData) {
	...
	ctxos.textBaseline = "middle";
	ctxos.textAlign = "left";
	ctxos.font = "80px Saria,Noto Sans SC";
	const dxsnm = ctxos.measureText(inputName.value || inputName.placeholder).width;
	if (dxsnm > 1500) ctxos.font = `${80/dxsnm*1500}px Saria,Noto Sans SC`;
	ctxos.fillText(inputName.value || inputName.placeholder, 700 * tween[8](range(qwqEnd.second * 1.25)) - 320, 145);
	ctxos.font = "30px Saria,Noto Sans SC";
	const dxlvl = ctxos.measureText(inputLevel.value || inputLevel.placeholder).width;
	if (dxlvl > 750) ctxos.font = `${30/dxlvl*750}px Saria,Noto Sans SC`;
	...
	ctxos.font = "22px Saria,Noto Sans SC";
